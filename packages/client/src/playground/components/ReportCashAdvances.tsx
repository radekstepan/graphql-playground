...
const ReportCashAdvances = () => {
  ...
  return (
    <div ref={componentRef} className="component">
      {!cashAdvances.isFetching && !!cashAdvances.data?.report.cashAdvances.length && (
        <div>Cash advances: ${cashAdvances.data.report.cashAdvances.reduce((acc, ca) => acc + ca.amount, 0)}</div>
      )}
      {!cashAdvances.isFetching && !cashAdvances.data?.report.cashAdvances.length && 'No cash advances'}
