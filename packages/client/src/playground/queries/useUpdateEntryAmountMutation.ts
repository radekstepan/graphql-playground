...
import { UPDATE_ENTRY_AMOUNT } from "../../queries";
import { type UpdateEntryAmountMutationVariables } from "../../__generated/graphql";
...
export const useUpdateEntryAmountMutation = () => {
  ...
  return useMutation({
    mutationFn: (variables: UpdateEntryAmountMutationVariables) => {
      setIsLoading(true);
      return gqlClient.request(UPDATE_ENTRY_AMOUNT, variables);
    },
    onSuccess: () => {
