...
export const ReportEntryQueryProvider: FC<{
  ...
  const {refetch} = useQuery({
    ...
    queryFn: () => {
      setIsLoading(true);
      return gqlClient.request(GET_ENTRY, {
        entryId,
    ...
    onSuccess: (data) => {
      if (data.entry.amount !== undefined) {
        includeFragmentsRef.current.delete('includeAmount');
        // NOTE: the report entries list is not modified.
        setQueryData(DataStatus.LATEST, keys.reportEntry.getReportEntryAmount(reportId, entryId), data.entry.amount);
      }
      if (data.entry.receipt !== undefined) {
        includeFragmentsRef.current.delete('includeReceipt');
        // NOTE: the report entries list is not modified.
        setQueryData(DataStatus.LATEST, keys.reportEntry.getReportEntryReceipt(reportId, entryId), data.entry.receipt);
      }
      if (data.entry.exceptions !== undefined) {
        includeFragmentsRef.current.delete('includeExceptions');
        setQueryData(DataStatus.LATEST, keys.report.getEntryExceptions(reportId, entryId), data.entry.exceptions);
      }
    },
    onSettled: () => {
